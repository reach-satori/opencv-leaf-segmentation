...
#filter noise to improve sorting speed, sort from largest to smallest
    ...
    # img = cv.bitwise_and(img, img, mask=otsu)
    return cropped
...
def get_area(img):
    ...
    # ret, proc = cv.threshold(proc, proc.max()*0.2, 255, cv.THRESH_BINARY+cv.THRESH_OTSU)
    ret, proc = cv.threshold(v, 0, 255, cv.THRESH_BINARY+cv.THRESH_OTSU)
    proc = cv.bitwise_not(proc)
    proc = cv.morphologyEx(proc, cv.MORPH_CLOSE, cv.getStructuringElement(cv.MORPH_ELLIPSE, (7,7)), iterations=2)
    proc = cv.erode(proc, cv.getStructuringElement(cv.MORPH_ELLIPSE, (7, 7)), iterations = 3)
    ...
    #One more step is neceesary to make sure we're getting only the plant contour
